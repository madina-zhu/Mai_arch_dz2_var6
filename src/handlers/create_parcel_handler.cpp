...
namespace handlers
{
      ...
      userver::server::request::RequestContext &) const
  {
    ...
    // Проверяем, существует ли отправитель в PostgreSQL
    if (!pg_storage_.GetUserById(dto.sender_id).has_value())
    {
    ...
    std::string parcel_id = storage_.CreateParcel(dto);

    if (parcel_id == components::MongoStorageComponent::constraintViolation)
    {
