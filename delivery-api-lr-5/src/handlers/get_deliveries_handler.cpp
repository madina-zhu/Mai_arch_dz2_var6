...
namespace handlers
{
        ...
        userver::server::request::RequestContext &) const
    {
        ...
        if (request.HasArg("tracking_number"))
        {
            ...
            auto delivery = cache_.SearchDeliveryByTrackingNumber(tracking_number);

            if (!delivery.has_value())
            {
                delivery = storage_.GetDeliveryByTrackingNumber(tracking_number);
                if (delivery.has_value())
                {
                    cache_.SaveDeliveryByTrackingNumber(tracking_number, *delivery);
                }
            }
        ...
        if (request.HasArg("sender_id"))
        {
            try
            {
                ...
                auto deliveries = cache_.SearchDeliveriesBySender(sender_id, from, to);

                if (!deliveries.has_value())
                {
                    deliveries = storage_.GetDeliveriesBySender(sender_id, to, from);
                    if (deliveries.has_value())
                    {
                        const auto key = "deliveries:sender:" + std::to_string(sender_id) + ":" +
                                         std::to_string(from) + ":" + std::to_string(to);
                        cache_.SaveDeliveriesSearchResult(key, *deliveries);
                    }
                }

                if (!deliveries.has_value())
                {
                    std::vector<models::dto::DeliveryResponse> empty;
                    return userver::formats::json::ToString(
                        userver::formats::json::ValueBuilder{empty}.ExtractValue());
                }
        ...
        if (request.HasArg("receiver_id"))
        {
            try
            {
                ...
                auto deliveries = cache_.SearchDeliveriesByReceiver(receiver_id, from, to);

                if (!deliveries.has_value())
                {
                    deliveries = storage_.GetDeliveriesByReceiver(receiver_id, to, from);
                    if (deliveries.has_value())
                    {
                        const auto key = "deliveries:receiver:" + std::to_string(receiver_id) + ":" +
                                         std::to_string(from) + ":" + std::to_string(to);
                        cache_.SaveDeliveriesSearchResult(key, *deliveries);
                    }
                }
