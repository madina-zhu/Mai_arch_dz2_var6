...
- 🔐 JWT-аутентификация
- ⚡ Оптимизация — кеширование ответов и ограничение частоты запросов (Rate Limiting)
```
...
В рамках данного этапа развития проекта была реализована стратегия повышения производительности и устойчивости системы к высоким нагрузкам.

### 1. Кеширование (Caching)
Для снижения нагрузки на базы данных (особенно на сложные запросы поиска в MongoDB) внедрено кеширование с использованием **Redis**.

- **Стратегия:** Cache-Aside (Lazy Loading).
- **Кешируемые эндпоинты:**
  - `GET /properties?city=...&`, `GET /properties?min_price=...&max_price=...` — списки объектов доставки.
  - `GET /users?login=...` — данные пользователей.
- **TTL (Time To Live):**
  - Для списков доставки: **60 секунд** (баланс между актуальностью и снижением нагрузки).
  - Для профилей пользователей: **300 секунд**.
- **Инвалидация:** При создании (`POST`) объекта доставки соответствующие ключи кеша удаляются, чтобы пользователи не видели устаревшие данные дольше установленного TTL.

### 2. Ограничение частоты запросов (Rate Limiting)
Для защиты API от злоупотреблений (brute-force, DDoS, скрапинг) реализован механизм Rate Limiting.

- **Алгоритм:** Token Bucket.
- **Защищенные эндпоинты:**
  - `GET /properties` — защита от массового скачивания данных.
- **Лимиты:**
  - 100 запросов в минуту на поисковые запросы.
- **Ответы:** При превышении лимита возвращается статус `429 Too Many Requests` с заголовками `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`.

📄 **Подробное описание стратегии проектирования, анализ метрик и план тестирования производительности доступны в файле:** [performance_design.md](performance_design.md)
...
- Дизайн производительности (Кеширование и Rate Limiting): См. файл [performance_design.md](performance_design.md)