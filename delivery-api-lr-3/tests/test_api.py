...
def create_test_delivery_data(parcel_id: int, sender_id: int, receiver_id: int) -> Dict[str, Any]:
...
class TestUserEndpoints:
    ...
    def test_search_users_by_login(self, session: requests.Session, registered_user: Dict[str, Any]):
        response = session.get(
            f"{BASE_URL}/users",
            params={"from": 1, "to": 10, "login": registered_user["login"]},
            timeout=TIMEOUT
        )
        assert_success_response(response, 200)
        data = response.json()
    ...
    def test_search_users_by_name_mask(self, session: requests.Session, registered_user: Dict[str, Any]):
        response = session.get(
            f"{BASE_URL}/users",
            params={"from": 1, "to": 10, "name_mask": "Test*"},
            timeout=TIMEOUT
        )
        assert_success_response(response, 200)
...
class TestParcelEndpoints:
    ...
    def test_get_user_parcels(self, session: requests.Session, test_user_with_token: Dict[str, Any], created_parcel: dict):
        response = session.get(
            f"{BASE_URL}/parcels",
            params={"from": 1, "to": 10, "user_id": test_user_with_token["id"]},
            timeout=TIMEOUT
        )
        assert_success_response(response, 200)
        data = response.json()
...
class TestDeliveryEndpoints:
    ...
    def test_create_delivery_without_auth(self, session: requests.Session, created_parcel: dict):
        delivery_data = create_test_delivery_data(created_parcel["id"], 1, 2)
    ...
    def test_get_delivery_by_tracking_number(
        ...
        response = session.get(
            f"{BASE_URL}/deliveries",
            params={"from": 1, "to": 10, "tracking_number": tracking_number},
            timeout=TIMEOUT
        )
        assert_success_response(response, 200)
        data = response.json()
    ...
    def test_get_deliveries_by_sender(
        ...
        response = session.get(
            f"{BASE_URL}/deliveries",
            params={"from": 1, "to": 10, "sender_id": test_user_with_token["id"]},
            timeout=TIMEOUT
        )
        assert_success_response(response, 200)
        data = response.json()
    ...
    def test_get_deliveries_by_receiver(
        ...
        response = session.get(
            f"{BASE_URL}/deliveries",
            params={"from": 1, "to": 10, "receiver_id": second_user["id"]},
            timeout=TIMEOUT
        )
        assert_success_response(response, 200)
        data = response.json()
...
class TestEdgeCases:
    def test_full_workflow(self, session: requests.Session):
        ...
        delivery_data = create_test_delivery_data(parcel_id, sender_id, receiver_id)
        response = session.post(
            f"{BASE_URL}/deliveries",
            json=delivery_data,
            headers=headers,
