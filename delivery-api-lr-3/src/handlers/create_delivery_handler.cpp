...
namespace handlers
{
      ...
      userver::server::request::RequestContext &) const
  {
    ...
    if (delivery_id == components::PostgresStorageComponent::DATA_VIOLATION)
    {
      request.SetResponseStatus(userver::server::http::HttpStatus::kNotFound);
      models::dto::ErrorResponse error{"NOT_FOUND", "Parcel or receiver not found"};
      return userver::formats::json::ToString(
          userver::formats::json::ValueBuilder{error}.ExtractValue());
    }

    if (delivery_id == components::PostgresStorageComponent::CONSTRAINT_VIOLATION)
    {
