...
namespace handlers
{
      ...
      userver::server::request::RequestContext &) const
  {
    ...
    if (parcel_id == components::PostgresStorageComponent::DATA_VIOLATION)
    {
      request.SetResponseStatus(userver::server::http::HttpStatus::kNotFound);
      models::dto::ErrorResponse error{"NOT_FOUND", "Sender not found or invalid data"};
      return userver::formats::json::ToString(
          userver::formats::json::ValueBuilder{error}.ExtractValue());
    }

    if (parcel_id == components::PostgresStorageComponent::CONSTRAINT_VIOLATION)
    {
