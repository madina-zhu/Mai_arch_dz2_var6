...
namespace components
{
  ...
  std::vector<models::dto::ParcelResponse>
  MongoStorageComponent::GetParcelsByStatus(const std::string &status, int to, int from)
  {
    std::vector<models::dto::ParcelResponse> result;
    auto cursor = parcels_collection_.Find(
        userver::formats::bson::MakeDoc("status", status),
        userver::storages::mongo::options::Limit(to - from + 1),
        userver::storages::mongo::options::Skip(from - 1));
    result.reserve(to - from + 1);
    for (const auto &doc : cursor)
  ...
  std::vector<models::dto::DeliveryResponse>
  MongoStorageComponent::GetDeliveriesByStatus(const std::string &status, int to, int from)
  {
    std::vector<models::dto::DeliveryResponse> result;
    auto cursor = deliveries_collection_.Find(
        userver::formats::bson::MakeDoc("status", status),
  ...
  bool MongoStorageComponent::AddDeliveryEvent(const std::string &delivery_id,
                                               const std::string &status,
                                               const std::string &location,
                                               const std::string &comment)
  {
      ...
      auto result = deliveries_collection_.UpdateOne(
          ...
          userver::formats::bson::MakeDoc(
              "$push", userver::formats::bson::MakeDoc(
                           "events", userver::formats::bson::MakeDoc(
                                         "status", status,
    ...
    {
      LOG_ERROR() << "AddDeliveryEvent failed: " << e.what();
      return false;
    }
  }
      ...
      const models::dto::ParcelCreateRequest &req)
  {
    ...
    userver::formats::bson::ValueBuilder features_array;
    if (req.features)
    {
      for (const auto &feature : *req.features)
      {
        features_array.PushBack(feature);
      }
    }
    ...
    return userver::formats::bson::MakeDoc(
        ...
        "features", features_array.ExtractValue());
  }
